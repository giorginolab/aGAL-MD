...
The protein structure we used here is based on the [PDB 3GXT](rcsb.org/3d-view/3GXT/1) model, which underwent a reglycosilation step via [glycoshape.org](https://glycoshape.org/reglyco) 
...
```
With *n* being the number of jobs to be generated for each structure_replica (for example, we run this system for 1 μs and used 7 jobs)
